...
public class SqlServerlDbHelper {

	private static Logger LOGGER = Logger.getLogger(SqlServerlDbHelper.class.getName());
	private static SimpleDateFormat sdfy = new SimpleDateFormat(JsonUtils.DATE_FORMAT_YEAR_FIRST);
	
	Connection mCon;

	public SqlServerlDbHelper(Connection con) throws Exception {
		mCon = con;
	}
	...
	public <T extends Bean,P> DataSource<T> getBeans(BeanFilter filter) throws Exception {
		...
		String SqlCreateRow = "(SELECT "+tbl+".*, ROW_NUMBER() OVER (ORDER BY " +order.getString()+ ") rn FROM " 
				+ tbl + " " + tbl + " " + join.getString() +  " WHERE " + condition.getString()+") TMPTBL ";
		...
		if(filter.getLimit() != 0) {
			...
			SQL += " AND TMPTBL.rn >= " + offset + " AND TMPTBL.rn <=" + offsetInt;
		}
		
		/*hiepn 2016.10.04 - modified - begin*/
		if(order.getString().length() != 0) {
			SQL += " ORDER BY TMPTBL." + order.getString();
		}
	...
	/**
	 * @param filter
	 * @return count rows in select SQL Server 
	 * @throws Exception
	 */
	...
	private <T extends Bean>  void insertBatch(DataSource<T> beanArray) throws Exception {
		Class<T> beanClass = beanArray.getBeanClass();
		BeanSchema metaInfo = BeanSchema.loadSchema(beanClass);
//		System.out.println(metaInfo.getExtKeyCols().toArray()[0]); 
//		String extKey = metaInfo.getExtKeyCols().toArray()[0].toString();
